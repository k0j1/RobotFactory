  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <Card className="bg-stone-50 border-2 border-stone-200">
        <h3 className={`${theme.typography.h3} mb-4 text-stone-700`}>ポンコツロボット工房 v1.0.155 仕様まとめ</h3>
        ...
        <div className="space-y-6 text-sm text-stone-800">
          ...
          <section>
            ...
            <ul className="list-disc list-inside space-y-1 ml-2">
              ...
              <li><strong>音楽（ピアノ演奏）:</strong> 楽曲ごとに設定された難易度と、任意で選択するテンポ（簡単/普通/難しい）に応じて、ピアノの演奏を行います。ロボットの<strong>「賢さ (Int)」</strong>と<strong>「器用さ (Dex)」</strong>の値が高いほど、正確な鍵盤の押下とタイミング調整が行われ、より高い評価（EXCELLENTなど）を獲得しやすくなり、結果として高スコアを狙うことができます。
                <ul className="list-disc list-inside space-y-1 ml-4 mt-1">
                  <li><strong>3オクターブ（21白鍵）と和音（同時押し）対応:</strong> ピアノの鍵盤が低音から高音まで拡張され、演奏時は実際の周波数に基づいたピアノの音が鳴ります。実際の音楽同様のリズムと、複数鍵盤の同時押し（和音・アルペジオ）に対応しており、難易度の高い曲では一度に大量のスコアを獲得するチャンスがあります。</li>
                  ...
                  <li><strong>タイミング判定と複雑なスコア計算:</strong> 和音（複数の鍵盤）の同時押し時は、正しく押せた鍵盤の数だけスコアが倍増し、大技を決めることで一気にスコアを稼ぐことができます。各楽曲の最大コンボ数に応じた適切な目標スコアが自動的に設定されます。</li>
                  <li><strong>動的スピード変化:</strong> 高難易度の楽曲では、音楽のリズムや進行に合わせて流れてくるノーツの落下速度が細かく変動し、プレイヤーに揺さぶりをかける演出が入ります。</li>
                </ul>
              </li>
          ...
          <section>
            ...
            <ul className="list-disc list-inside space-y-1 ml-2">
              <li><strong>同行ロボ選択カードの工房イラスト背景（待機ロボットが並ぶ工房空間）:</strong> 遠征画面上部の「同行ロボ選択」カード背景に、愛嬌ある多様なロボットたちが作業台や充電ドックに整然と待機・並んでいる温かみのある工房イラストを統合。前面の横スライド切り替えや各機体選択ボタンの視認性・操作性を損なわないよう、半透明ホワイトグラデーションオーバーレイと柔らかなガラス調エフェクト（backdrop-blur）を施し、工房の豊かな世界観と快適な操作性を両立しました。</li>
              <li><strong>ガレージ＆ロボット背景アート（薄表示・可読性最適化）:</strong> ユーザー提示のガレージ構図（オープンした巻き上げ式シャッター、大型金属製スチールシェルフ、整理整頓されたダンボール箱やパーツストック、右側の赤い大型ツールチェスト・作業台、壁掛け工具、外光が差し込む磨かれたコンクリート床）に基づき、<strong>車をガレージ中央に佇むポンコツロボットへ置き換えた背景アート</strong>をダッシュボードに統合。前面の文字や数値（所持金、機体数、納品実績、修理キット、ミッション情報など）がくっきりと視認できるよう、背景画像の透過度を控えめ（opacity 0.22）に薄くし、柔らかな乳白色グラデーションオーバーレイ（from-[#fcf9f5]/75 via-[#fcf8f3]/55 to-[#f8f1e5]/80）を重ねることで、文字の見えにくさを完全に解消しながら上品なガレージの空気感を演出しました。</li>
