              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                ...
                <div className="bg-white p-3 rounded-lg border border-stone-200 space-y-1.5">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-stone-600">
                    ...
                    <li><strong>Googleアカウント連携 &amp; クラウドデータ同期 (DB v0.12):</strong> Googleアカウントでログインした場合、オンライン上にゲームデータ（工房ステータス、所持機体、パーツ、遠征、ミニゲーム成績・宝箱数等）が自動的に保存・同期されます。所持パーツテーブル（<code>user_parts</code>）は個別カラム（vitality・power・defense・agility・dexterity・intelligence・attribute・rarity・メイン/サブ素材マスターID等）へ構造化され、ロボット組立テーブル（<code>active_robot_assemblies</code>）は冗長な機体ステータスカラム（hp・power・defense・agility・dexterity・intelligence）や戦闘成績（battle_stats）、旧JSONデータ（result_robot_data）が削除・スリム化され、頭・胴・腕・脚の各パーツ情報（<code>user_parts(id)</code>を外部キー参照）と機体基本属性（機体ID・機体名・現在HP・最大HP・価値・作成日時）のみで管理される高精度・高効率なリレーショナル構造へと正規化されています。また依頼受注テーブル（<code>active_requests</code>）においても、個別カラムで保持している依頼ID・ランク・報酬G・期限は<code>request_data</code>から削除・除外され、テーブル列と分離・最適化されています。ミニゲーム状況テーブル（<code>user_minigame_status</code>）には挑戦回数・勝利数・獲得エレメント数に加え<strong>宝箱獲得数（<code>chests_count</code>）</strong>および未開封所持宝箱数が永続化されます。別の端末からでも同じアカウントで続きからプレイが可能です。ゲストプレイ時は端末内に独立してデータが保護されます。</li>
