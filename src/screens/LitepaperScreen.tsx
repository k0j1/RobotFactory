              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                <div className="bg-white p-2.5 rounded-lg border border-stone-200">
                  ...
                  <p className="text-stone-600">
                    ...
                    <br />
                    ・<strong>🎨 ランク連動カラーシステム:</strong> ロボットが装備するビームサーベルの光刃およびビームシールドのエネルギー力場は、ランクに応じて<strong>★1 Common (ホワイト・シルバー) → ★2 Uncommon (エメラルド・グリーン) → ★3 Rare (アクア・スカイブルー) → ★4 Epic (アメジスト・パープル) → ★5 Legendary (ソーラー・ゴールド)</strong>へとダイナミックに変化！バトル中の待機・突進・被弾モーションはもちろん、通常攻撃のスラッシュ光刃や専用奥義【星断オメガクロス】・【エネルギーシールド防御】の巨大エフェクトの色も装備ランクのカラーに完全同期します。
                    <br />
                    ・<strong>🎊 解放・ランクアップ時の祝祭紙吹雪演出:</strong> 武装の新規購入時およびランクアップ強化時には、画面いっぱいに華やかな紙吹雪が舞い散る祝祭演出とファンファーレが発動し、強化達成感をドラマチックに盛り上げます。
                    <br />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                ...
                <div className="bg-white p-2.5 rounded-lg border border-stone-200">
                  ...
                  <p className="text-stone-600">
                    ...
                    <br />
                    <strong>データベース構造化（user_item テーブル）:</strong> 修理キット所持数（<code>repair_kit</code>）、各ランク宝箱の所持数（<code>bronze_chest</code>, <code>silver_chest</code>, <code>gold_chest</code>, <code>mythic_chest</code>）、およびバトルエレメント数（<code>element</code>）は、専用の <code>user_item</code> テーブルへと独立・正規化されて永続保存されます。従来の巨大JSON（<code>user_save_data</code> テーブル）からはこれらのデータが物理的に削除され、アイテムデータの整合性と高速な同期が保証されています。
                    <br />
