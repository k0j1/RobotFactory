              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                ...
                <div className="bg-white p-3 rounded-lg border border-stone-200 space-y-1.5">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-stone-600">
                    ...
                    <li><strong>Googleアカウント連携 &amp; クラウドデータ同期 (DB v0.18):</strong> Googleアカウントでログインした場合、オンライン上にゲームデータ（工房ステータス、所持機体、パーツ、遠征、ミニゲーム成績・宝箱数等）が自動的に保存・同期されます。<code>users</code>テーブルの<code>google_id</code>を親キーとして、全23テーブル（<code>user_item</code>, <code>user_material</code>, <code>user_minigame_status</code>, <code>user_parts</code>, <code>user_robots</code>, <code>user_workshop_status</code>, <code>save_data</code>, <code>minigame_rankings</code>, <code>daily_cleared_minigame</code>, <code>complete_part_crafts</code>等の各種active/completeテーブル）の<code>user_id</code>に厳格な外部キー制約（CASCADE）が設定され、データの参照整合性と安全性が保証されています（旧<code>complete_parts</code>テーブルは廃止され、パーツ製造履歴は<code>complete_part_crafts</code>へ統合・正規化）。所持パーツテーブル（<code>user_parts</code>）は個別カラム（vitality・power・defense・agility・dexterity・intelligence・attribute・rarity・メイン/サブ素材マスターID等）で構造化管理され、保存時に未装備パーツ（<code>is_equipped = 0</code>）や既存レコードの<code>created_at</code>作成日時が安全に保護され、余計な一括削除や意図しない日時の上書きが発生しないよう最適化されています。パーツ製造完了時には、<code>active_part_crafts</code>の削除、<code>complete_part_crafts</code>への製造パーツ情報（<code>result_part_data</code>）付き履歴追加、および<code>user_parts</code>テーブルへの新パーツ追加が単一トランザクション内で不可分に実行され、異常発生時には全ロールバックされます。また、ミニゲーム/演習クリア制限テーブル（<code>daily_cleared_minigame</code>）により機体・ゲーム・難易度ごとのクリア状況が記録され、毎朝9:00（JST）のデイリーリセットを迎えるまで同機体の再クリアが制限されます。</li>
