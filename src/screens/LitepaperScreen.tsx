  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <Card className="bg-stone-50 border-2 border-stone-200">
        <h3 className={`${theme.typography.h3} mb-4 text-stone-700`}>ポンコツロボット工房 v1.0.160 仕様まとめ</h3>
          ...
          <section>
            ...
            <ul className="list-disc list-inside space-y-1 ml-2">
              ...
              <li><strong>音楽（ピアノ演奏）:</strong> 楽曲ごとに設定された難易度と、任意で選択するテンポ（簡単/普通/難しい）に応じて、ピアノの演奏を行います。ロボットの<strong>「賢さ (Int)」</strong>と<strong>「器用さ (Dex)」</strong>の値が高いほど、正確な鍵盤の押下とタイミング調整が行われ、より高い評価（EXCELLENTなど）を獲得しやすくなり、結果として高スコアを狙うことができます。
                <ul className="list-disc list-inside space-y-1 ml-4 mt-1">
                  <li><strong>5オクターブ（35白鍵）と和音（同時押し）対応:</strong> ピアノの鍵盤が低音(C2)から高音(B6)までの5オクターブ（35鍵）へと大きく拡張され、より幅広い音域でのダイナミックな演奏体験を実現しました。実際の音楽同様のリズムと、複数鍵盤の同時押し（和音・アルペジオ）に対応しており、難易度の高い曲では一度に大量のスコアを獲得するチャンスがあります。</li>
                  <li><strong>ロングノーツ（押す長さ）とリッチな合成音:</strong> エンベロープと複数オシレーターを組み合わせたリッチなピアノ風シンセサイザーを実装し、実際のピアノに近い音色と余韻を再現。さらにノーツの長さに応じて鍵盤を押し続ける「ロングノーツ」のゲーム要素を追加し、長く押すほどスコアが増幅するシステムを導入しました。</li>
                  ...
                  <li><strong>難易度と複雑さの変化:</strong> 難易度設定は以下のように進化しました。「簡単（選択した曲の音源とほぼ同じ音を奏でる原曲再現）」「普通（超絶技巧が連続する、人間が弾くことができる限界レベルのアレンジ）」「難しい（音源を損なわずに、人2人がピアノを連弾しているレベルで元音源をアレンジした最高峰の超絶譜面）」と、原曲の美しさを保ちながら限界を超えたピアノ演奏体験を提供します。</li>
                  <li><strong>タイミング判定と複雑なスコア計算:</strong> 和音（複数の鍵盤）の同時押し時は、正しく押せた鍵盤の数だけスコアが倍増し、大技を決めることで一気にスコアを稼ぐことができます。さらにロングノーツを正確に押し続けることでスコアが大きく伸びます。各楽曲の最大コンボ数とノーツの長さに応じた適切な目標スコアが自動的に設定されます。</li>
