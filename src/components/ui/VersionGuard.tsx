...
export function VersionGuard() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className={`w-full max-w-md ${theme.colors.surface} border border-sky-500/50 rounded-xl p-6 shadow-2xl relative overflow-hidden`}>
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="text-xl font-bold text-sky-400 mb-2">更新あり</h2>
          <p className="text-stone-300 text-sm mb-6 whitespace-pre-wrap">
            {errorMsg || '新しいバージョンが利用可能です。最新の状態にするため、ページを更新してください。'}
          </p>
          <button
            onClick={() => window.location.reload()}
            className="flex items-center gap-2 px-6 py-3 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-lg transition-colors"
          >
