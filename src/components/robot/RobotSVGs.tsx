...
export const ArmsStar2SVG = ({ color, viewBox = "-18 -34 100 100", className }: SVGProps) => {
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox={viewBox} className={className} shapeRendering="crispEdges" width="100%" height="100%">
      {/* LEFT ARM (付け根 (12, 8) を中心にサイズを半分に縮小) */}
      <g transform="translate(12, 8) scale(0.5) translate(-12, -8)">
      ...
      {/* RIGHT ARM (付け根 (52, 8) を中心にサイズを半分に縮小) */}
      <g transform="translate(52, 8) scale(0.5) translate(-52, -8)">
...
export const LegsStar2_2SVG = ({ color, viewBox = "-230 -450 760 760", className }: SVGProps) => {
  const pal = getAttributePalette(color);
  const uid = React.useId().replace(/:/g, '');
...
export const LegsStar2_3SVG = ({ color, viewBox = "-230 -450 760 760", className }: SVGProps) => {
  const pal = getAttributePalette(color);
  const uid = React.useId().replace(/:/g, '');
...
export const LegsStar2_4SVG = ({ color, viewBox = "-230 -450 760 760", className }: SVGProps) => {
  const pal = getAttributePalette(color);
  const uid = React.useId().replace(/:/g, '');
